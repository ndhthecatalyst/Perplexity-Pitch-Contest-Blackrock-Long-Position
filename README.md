# Perplexity Pitch Contest — BlackRock Long Position

**Nicholas Hawkins | Hawkins Holdings | April 2026**

---

## Investment Thesis

> *BlackRock (NYSE: BLK) is the highest-quality capital allocation in the asset management sector for 2026 — a fee-compounding platform at the intersection of indexed investing, private markets, and financial operating-system infrastructure.*

Currently trading ~21% below its 52-week high, BLK offers a technically attractive entry point with **35%+ consensus upside** to the analyst average price target of ~$1,300.

---

## Repository Structure

```
├── models/
│   └── BLK_DCF_Model_Hawkins_Holdings_2026.xlsx   ← Main DCF model (6 sheets)
├── pitch-materials/
│   ├── blk_flagship_asset_allocation.html          ← Interactive pitch deck
│   └── BlackRock-Initial-Inquiry.pdf               ← Initial research brief
├── docs/
│   ├── BLK-Investment-Analysis-Guidance.pdf        ← Analysis framework
│   ├── BlackRock-BLK-2026-Perplexity-Implementation-Plan.pdf
│   ├── BlackRock-BLK-DCF-Evidence-Ledger.docx
│   └── BlackRock-BLK-Financial-Profile-FY2022-FY2025.docx
├── README.md
└── SOURCES.md
```

---

## DCF Model — Sheet Index

| Sheet | Description |
|---|---|
| **Cover** | Navigation, key metrics snapshot, investment thesis |
| **Income Statement** | FY2022A–FY2027E: Revenue, Adj. EBITDA, Adj. EPS, AUM |
| **FCF Bridge** | NOPAT → FCFF → FCFE with all adjustments |
| **WACC Calculator** | CAPM cost of equity, cost of debt — Bear/Base/Bull |
| **DCF Valuation** | Terminal value (perpetuity), intrinsic value per share |
| **Sensitivity Tables** | WACC × terminal growth rate → IV/share (3-color heatmap) |

---

## Key Financial Data (FY2025 Actuals)

| Metric | Value | YoY |
|---|---|---|
| AUM | $14.04T | +21.6% |
| Revenue | $24.22B | +18.7% |
| Adj. Operating Margin | 44.1% | −40bps |
| Adj. EPS | $48.09 | +10.3% |
| Full-Year Net Inflows | $698.3B | Record |
| Cash & Equivalents | $11.5B | — |

---

## Valuation Scenarios

| Scenario | WACC | Terminal g | Intrinsic Value / Share |
|---|---|---|---|
| Bear | 11.0% | 2.0% | ~$750–850 |
| Base | 9.5% | 2.5% | ~$1,150–1,350 |
| Bull | 8.0% | 3.25% | ~$1,550–1,800 |

*See `Sensitivity Tables` sheet for full WACC × g matrix across all scenarios.*

---

## Three-Layer Conviction Stack

1. **AUM Compounding Machine** — iShares #1 ETF globally, $698B record inflows, 3–5% organic growth target
2. **Strategic Platform Transformation** — GIP (Oct 2024) + Preqin (Mar 2025) + HPS (Jul 2025) pivot BLK to alternatives + data
3. **Aladdin Technology Moat** — Infrastructure layer for $20T+ in institutional assets; Preqin integration unlocks private-markets data monetization

---

## Analyst Consensus (April 2026)

- **Rating:** Strong Buy (15 Buys, 2 Holds, 0 Sells)
- **Average PT:** ~$1,300
- **High PT:** $1,550
- **Current Price:** ~$958 (April 3, 2026)
- **Implied Upside to Avg. PT:** ~35.7%

---

## Sources

See [SOURCES.md](SOURCES.md) for full citation index.

Primary sources:
- [BlackRock Q4 2025 Earnings Release (Jan 15, 2026)](https://www.blackrock.com/corporate/newsroom/press-releases/article/corporate-one/press-releases/blackrock-reports-fourth-quarter-2025)
- [SEC 10-K Filings (EDGAR)](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0001364742)
- [StockAnalysis.com — BLK Financials](https://stockanalysis.com/stocks/blk/financials/)
- [GuruFocus WACC](https://www.gurufocus.com/term/wacc/BLK)
- [AlphaSpread Discount Rate](https://www.alphaspread.com/security/nyse/blk/discount-rate)
- [StockAnalysis — BLK Forecast](https://stockanalysis.com/stocks/blk/forecast/)

---

## Disclaimer

*This repository is prepared for the Perplexity Pitch Contest (April 2026) and does not constitute investment advice. All projections are based on publicly available data and analyst consensus estimates.*

---

*Built with Perplexity + Claude — demonstrating AI-augmented institutional-grade investment analysis.*
